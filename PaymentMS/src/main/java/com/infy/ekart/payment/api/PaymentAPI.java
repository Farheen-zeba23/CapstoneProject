...
public class PaymentAPI {
	...
	public ResponseEntity<String> payForOrder(
		...
		String message=environment.getProperty("paymentAPI.TRANSACTION_SUCCESSFUL_ONE")+transactionDTO.getTotalPrice()+
				environment.getProperty("paymentAPI.TRANSACTION_SUCCESSFUL_ONE")+
				transactionDTO.getOrder().getOrderId()+environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFUL_THREE")+transactionDTO.getTransactionId();
	...
	public ResponseEntity<String> payForOrderFallback(String customerEmailId, TransactionDTO transactionDTO,
		...
		String msg ="";
		if(exception.getMessage().equals("Payment.Transaction_failed_cw_not_matching"))
		     msg="Payment.Transaction_failed_cw_NOT_Matching";
		else
			msg="PaymentAPI.PAYMENT_FAILURE_FALLBACK";
